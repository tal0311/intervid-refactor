...
async function signup(userCred) {
  const { accessToken, refreshToken, user } = await httpService.post(
    `${BASE_URL}signup`,
    userCred
  );
...
async function verifyEmail(code) {
  const { accessToken, refreshToken, user } = await httpService.put(
...
async function verifyCode(enteredCode) {
  const { isVerified } = await httpService.put(`${BASE_URL}verify-code`, {
    enteredCode,
  });
  httpService.mount401Interceptor();
  return isVerified;
...
async function getNewToken() {
  ...
  const { accessToken, refreshToken } = await httpService.post(
    `${BASE_URL}token`,
    {
