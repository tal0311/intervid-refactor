export default {
  ...
  methods: {
    setModalPosition() { // implemented context menu like behaviour - on component that uses modalmixin have a mousePos state - starts at null and on activation receieves {x: pageX, y: pageY} - thus the if else statement below.
      ...
      if (this.mousePos) {
        ...
        this.mousePos.x - this.modalWidth < 0 ? '10px' : this.mousePos.x - this.modalWidth + 'px' :
        bodyBounding.width - this.mousePos.x - this.modalWidth < 0 ? '10px' : bodyBounding.width - this.mousePos.x - this.modalWidth + 'px'
      } else {
        ...
        this.modalInlineStart = this.isEnglish ?
          this.modalPos.right - this.modalWidth < 0 ? '10px' : this.modalPos.right - this.modalWidth + 'px' :
