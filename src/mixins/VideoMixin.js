...
export default {
  ...
  methods: {
    ...
    checkIfSupported() {
      ...
      loggerService.info(
        "[VideoMixin] [checkIfSupported] Media Devices are not supported in applicant's browser"
      );
    ...
    addListeners() {
      ...
      navigator.mediaDevices.ondevicechange = async (ev) => {
        const connectedDevicesCount =
          this.audioDevices?.length + this.videoDevices?.length;
        await this.loadDevices();
        if (
          this.videoDevices.length + this.audioDevices.length !==
          connectedDevicesCount
        ) {
          loggerService.info(
            "[VideoMixin] [ondevicechange] Media devices chaneged"
          );
      ...
      if (this._mediaRecorder) {
        ...
        loggerService.info(
          "[VideoMixin] [addListeners] Media recorder event listeners added"
        );
      }
    },
    ...
    async verifyPermissions() {
      ...
      if (!navigator.permissions) {
        loggerService.info(
          "[VideoMixin] [verifyPermissions] There is no navigator.permissions"
        );
        return;
      }
      try {
        ...
        if (cameraPerm.state === "prompt") {
          ...
          loggerService.info(
            "[VideoMixin] [verifyPermissions] Candidate has pre denied camera access",
            cameraPerm
          );
        ...
        loggerService.error(
          "[VideoMixin] [verifyPermissions] Could not query permission for camera",
          err
        );
      }
      ...
      try {
        ...
        if (micPerm.state === "prompt") {
          ...
          loggerService.info(
            "[VideoMixin] [verifyPermissions] Candidate has pre denied microphone access",
            micPerm
          );
        ...
        loggerService.error(
          "[VideoMixin] [verifyPermissions] Could not query permission for microphone",
          err
        );
      }
    },
    ...
    async handleCameraPermissionChange(e) {
      ...
      loggerService.info(
        "[VideoMixin] [handleCameraPermissionChange] Camera permissions has changed"
      );
      ...
      if (newState === "denied") {
        loggerService.info(
          "[VideoMixin] [handleCameraPermissionChange] Applicant has denied camera access",
          e.target
        );
        this.isStreaming = false;
        this.addVideoError(videoErrorMap.DENIED_CAM_ACCESS);
        ...
        if (
          this._cameraPermissionState === "denied" &&
          this._micPermissionState === "granted"
        ) {
          loggerService.info(
    ...
    async handleMicPermissionChange(e) {
      if (e.type !== "change") return;
      loggerService.info(
        "[VideoMixin] [handleMicPermissionChange] Microphone permissions has changed"
      );
      ...
      if (newState === "denied") {
        loggerService.info(
          "[VideoMixin] [handleMicPermissionChange] Applicant has denied microphone access",
          e.target
        );
        this.isStreaming = false;
        this.addVideoError(videoErrorMap.DENIED_MIC_ACCESS);
        ...
        if (
          this._micPermissionState === "denied" &&
          this._cameraPermissionState === "granted"
        ) {
          loggerService.info(
    ...
    addNetworkListener() {
      ...
      if (connection) {
        loggerService.info(
          "[videoMixin] [addNetworkListener] Navigator.connection event listener added"
        );
        ...
        loggerService.info(
          "[videoMixin] [addNetworkListener] Offline/online event listener added"
        );
      }
    },
    ...
    removeNetworkListener() {
      ...
      loggerService.info(
        "[videoMixin] [removeNetworkListener] Network listeners removed"
      );
    },
    ...
    async initVideoStream() {
      ...
      try {
        ...
        loggerService.error(
          "[videoMixin] [initVideoStream] Failed to create new stream"
        );
    ...
    async handleInitVideoErr(err) {
      loggerService.error(
        "[videoMixin] [handleInitVideoErr] error code",
        err.code
      );
      loggerService.error(
        "[videoMixin] [handleInitVideoErr] error name",
        err.name
      );
      loggerService.error(
        "[videoMixin] [handleInitVideoErr] error message",
        err.message
      );
      loggerService.error(
        "[videoMixin] [handleInitVideoErr] _micPermissionState",
        this._micPermissionState
      );
      loggerService.error(
        "[videoMixin] [handleInitVideoErr] _cameraPermissionState",
        this._cameraPermissionState
      );
      ...
      switch (err.code) {
        case 0: {
          if (err.name === "NotReadableError") {
            loggerService.error(
              "[videoMixin] [handleInitVideoErr] Could not start video source",
              { err }
            );
            ...
            if (err.message === "Permission denied by system") {
              loggerService.error(
                "[videoMixin] [handleInitVideoErr] Permission denied by system",
                { err }
              );
              this.addVideoError(videoErrorMap.MEDIA_DENIED_BY_SYSTEM);
            } else {
              loggerService.error(
                "[videoMixin] [handleInitVideoErr] Permission denied",
                { err }
              );

              if (this._micPermissionState !== "granted")
                this.addVideoError(videoErrorMap.DENIED_MIC_ACCESS);
              if (this._cameraPermissionState !== "granted")
                this.addVideoError(videoErrorMap.DENIED_CAM_ACCESS);
            }
          }
          break;
        ...
        case 8: {
          try {
            ...
            loggerService.error(
              "[videoMixin] [handleInitVideoErr] Camera not found",
              { err }
            );
          ...
          try {
            ...
            loggerService.error(
              "[videoMixin] [handleInitVideoErr] Microphone not found",
              { err }
            );
        ...
        case 20: {
          if (err.name === "AbortError") {
            loggerService.error(
              "[videoMixin] [handleInitVideoErr] Camera busy",
              { err }
            );
            this.addVideoError(videoErrorMap.CAMERA_BUSY);
          }
    ...
    removeVideoError(errorToRemove) {
      if (this.videoErrors.some((err) => err.type === errorToRemove.type)) {
        this.videoErrors = this.videoErrors.filter(
          (error) => error.type !== errorToRemove.type
        );
        loggerService.info(
          "[videoMixin] [removeVideoError] Video error removed",
          { errorToRemove }
        );
        if (this.selectedError?.type === errorToRemove.type)
    ...
    onSelectDevice(device) {
      ...
      if (device.type === "audioinput")
        this.selectedDevices.audio.deviceId = device.id;
    ...
    startVideoRecording() {
      try {
        ...
        loggerService.error(
          "[videoMixin] [startVideoRecording] Could not start video media recorder",
          e
        );
      }
    },
