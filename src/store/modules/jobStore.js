...
export const job = {
  ...
  actions: {
    async loadJobs({ commit, state, rootGetters }, { filterBy, sort, shouldGather = false }) {
      ...
      try {
        ...
        await commit('setJobs', { jobs, pageCount, filteredJobCount, totalJobCount })
      } catch (err) {
    ...
    async loadApplicants({ commit, state, rootGetters }, { filterBy = {}, sort = {}, shouldGather = false }) {
      commit('setIsFetching', true)
      try {
        ...
        commit('setApplicants', { applicants, filteredApplicantCount, pageCount })
      } catch (err) {
    ...
    async toggleArchiveApplicant({ dispatch, commit, state }, { applicants, isAllSelected }) {
      const cachedApplicants = state.applicants
