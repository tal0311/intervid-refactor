  ...
  >
    ...
    <div @click.stop="" class="status">
      <status-dropdown
        ...
        :isShowArchived="filterBy.showArchived"
      />
    </div>
  ...
  methods: {
    ...
    goToDetails() {
      if (
        ...
        (this.isFreeUser && !this.applicant.isFree)
      )
        return;
      this.$router.push(
        `/backoffice/details/${this.applicant.jobInfo.jobId}/${this.applicant.id}`
      );
    },
