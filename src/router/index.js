...
const routes = [
  {
    ...
    component: () =>
      import(
        /* webpackChunkName: "interview-app" */ "./views/interview/InterviewApp.vue"
      ),
    meta: {
      public: true,
    },
...

  {
    path: "/",
    component: () =>
      import(/* webpackChunkName: "back-office" */ "./views/MainApp.vue"),
    children: [
      {
        name: "Login",
        path: "/login",
...
router.beforeEach(async (to, from, next) => {
  ...
  const isToMidInterview = to.matched.some(
    (match) =>
      match.path === "/interview/:jobId/inprogress" ||
      match.path === "/interview/:jobId/end"
  );
  const isFromEnd = from.matched.some(
    (match) => match.path === "/interview/:jobId/end"
  );
  const isToInterview = to.matched.some(
    (match) => match.path === "/interview/:jobId/inprogress"
  );
  if (
    (isToMidInterview && (!applicant || !applicant.info)) ||
    (isFromEnd && isToInterview)
  ) {
