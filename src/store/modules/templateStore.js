...
export const template = {
  ...
  mutations: {
    ...
    updateTemplate(state, { template }) {
      const templateIdx = state.templates.findIndex(
  ...
  actions: {
    async loadTemplates({ commit }) {
      ...
      try {
        ...
        loggerService.error(
          "[templateStore] [loadTemplates] Failed to load templates",
          err
        );
      } finally {
    ...
    async loadDefaultTemplates({ rootGetters, state, commit }) {
      try {
        ...
        loggerService.error(
          "[templateStore] [loadDefaultTemplates] Failed to load default templates",
          err
        );
      }
    },
    ...
    async loadTemplate({ commit }, { templateId }) {
      ...
      try {
        ...
        loggerService.error(
          "[templateStore] [loadTemplate] Failed to load template",
    ...
    async saveTemplate({ commit, dispatch, rootGetters }, { template }) {
      ...
      try {
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.save("template") },
          { root: true }
        );
      } catch (err) {
        const user = rootGetters["job/userStore"].loggedInUser;
        loggerService.error(
          "[templateStore] [saveTemplate] Failed to save template",
          template?._id,
          "for",
          user._id,
          err
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.failSave("template") },
          { root: true }
        );
      }
    },
    ...
    async removeTemplate({ dispatch, commit, rootGetters }, { templateId }) {
      try {
        ...
        const user = rootGetters["job/userStore"].loggedInUser;
        loggerService.error(
          "[templateStore] [removeTemplate] Failed to remove template",
          templateId,
          "for",
          user._id,
          err
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.failRemove("template") },
          { root: true }
        );
      }
    },
    ...
    async toggleArchivedTemplate({ commit, dispatch }, { template }) {
      ...
      try {
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.failArchive("template") },
          { root: true }
        );
      }
    },
    ...
    toggleIsDefault({ dispatch, getters }, { template }) {
      const prevDefaultTemplate = getters.templates.find(
        (temp) => temp.isDefault
      );
