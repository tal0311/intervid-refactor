    ...
    <div class="applicant-form-content">
        ...
        <div class="content">
          ...
          <div class="greeting" :class="{ 'no-cover': !coverUrl }">
            ...
            <div class="time-container">
              ...
              <span class="color">{{
                `${totalDuration} ${getTrans("minutes")}`
              }}</span>
      ...
      <form ref="form" novalidate @submit.prevent="validateForm">
        ...
        <div class="inputs">
          ...
          <div
            :class="{ disabled: !applicantInfo.fName || !applicantInfo.lName }"
          >
            <cv-upload
              @uploaded="onCvUploaded"
              :errors="errors"
              :applicantCvName="applicantFullName"
            />
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    totalDuration() {
      return this.job.quests.reduce(
        (acc, quest) => (acc += quest.timeLimit),
        0
      );
    },
  ...
  components: { CvUpload, Application },
};
</script>
