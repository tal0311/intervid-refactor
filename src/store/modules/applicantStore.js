  ...
  actions: {
    async loadJob({ commit, state }, { jobId }) {
      try {
        ...
        loggerService.error(
          "[applicantStore] [loadJob] Failed to load job",
          jobId,
          err
        );
      } finally {
    ...
    async loadApplicant({ commit }, { applicantId }) {
      ...
      try {
        ...
        // commit('setInterviewErr', err)
        loggerService.error(
          "[applicantStore] [loadApplicant] Failed to load applicant",
          applicantId,
          err
        );
      } finally {
    ...
    async addApplicant({ commit, dispatch, state }, { applicant }) {
      try {
        dispatch(
          ...
          activityMap.applicant({
            type: "",
            desc: `submitted to job ${state.job.info.title} `,
          }),
          { root: true }
        );
        const addedApplicant = await applicantService.add(
          applicant,
          state.job._id
        );
    ...
    async updateApplicant({ commit, dispatch, state }, { applicant }) {
      try {
        dispatch(
          "activity/addActivity",
          activityMap.applicant({ type: "update", desc: `of ` }),
          { root: true }
        );
