<template>
  <section
    ref="playerContainer"
    class="video-player"
    :class="{ 'full-screen': playerState.isFullScreen }"
  >
    ...
    >
      <button class="material-icons" @click="toggleIsDraggableShown">
        close
      </button>
      <video
        :src="secVideoSrc"
        :class="{ cover: secVideoName === 'faceUrl' }"
        ref="secVideo"
        playsinline
      >
        Your browser does not support the video tag.
  ...
  async mounted() {
    ...
    document.onfullscreenchange = () => {
      if (!document.fullscreenElement)
        this.setPlayerState("isFullScreen", false);
    };
  },
  ...
  methods: {
      ...
      const videosPrms = elVideos.map((elVideo) => {
        return new Promise((resVid) => {
          elVideo.onloadedmetadata = () => {
            const totalDuration =
              elVideo.duration === Infinity && this.resTime
                ? this.resTime / 1000
                : elVideo.duration;
    ...
    addBufferListeners() {
      ...
      this.elMainVideo.oncanplaythrough = () => {
        ...
        if (!this.secVideoSrc || !this.isSecVideoWaiting)
      ...
      this.bufferInterval = setInterval(() => {
        ...
        const mainVideoProgress = this.elMainVideo.buffered.end(
          mainVideoBufferedLength - 1
        );
        ...
        if (this.secVideoSrc) {
          ...
          const secVideoProgress = this.elSecVideo.buffered.end(
            secVideoBufferedLength - 1
          );
