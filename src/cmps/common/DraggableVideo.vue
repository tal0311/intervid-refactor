...
export default {
  ...
  methods: {
    ...
    calcLastBoundaries() {
      const parent =
        this.$refs.draggableContainer.offsetParent.getBoundingClientRect();
    ...
    dragMouseDown(event) {
      ...
      if (!this.mousePos.clientX)
        this.mousePos.clientX = event.targetTouches[0].clientX;
      if (!this.mousePos.clientY)
        this.mousePos.clientY = event.targetTouches[0].clientY;
    ...
    isOutOfBoundaries() {
      ...
      if (
        parseInt(elContainer.style.left) >
        elContainer.offsetParent.clientWidth - elContainer.offsetWidth
      ) {
        elContainer.style.left =
          elContainer.offsetParent.clientWidth - elContainer.offsetWidth + "px";
        isOutOfBoundary = true;
      }
      if (
        parseInt(elContainer.style.top) >
        elContainer.offsetParent.clientHeight - elContainer.clientHeight
      ) {
        elContainer.style.top =
          elContainer.offsetParent.clientHeight -
          elContainer.clientHeight +
          "px";
        isOutOfBoundary = true;
      }
    ...
    elementDrag(event) {
      ...
      if (isTouchEvent)
        movementX = this.mousePos.clientX - event.targetTouches[0].clientX;
      if (isTouchEvent)
        movementY = this.mousePos.clientY - event.targetTouches[0].clientY;
    ...
    rePositionRelatively() {
      ...
      const topPercent =
        parseFloat(elContainer.style.top || 0) /
        this.lastBoundaries.parent.height;
      const leftPercent =
        parseFloat(elContainer.style.left || 0) /
        this.lastBoundaries.parent.width;
