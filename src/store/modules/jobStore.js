...
export const job = {
  ...
  mutations: {
    setJobsInfo(
      state,
      { applicantCount, applicantsTitles, jobsTitles, jobCount }
    ) {
    ...
    setApplicants(
      state,
      { applicants, pageCount, filteredApplicantCount, applicantCount }
    ) {
    ...
    addApplicant(state, { applicant, jobId }) {
    ...
    updateApplicants(state, { applicants }) {
      if (applicants && state.applicants) {
        for (const applicant of applicants) {
          var appIdx = state.applicants.findIndex(
      ...
      if (applicants && state.applicants) {
        for (const applicant of applicants) {
          state.applicants = state.applicants.filter(
            (_applicant) => _applicant.id !== applicant.id
          );
        }
      }
    },
    ...
    archiveApplicants(state, { applicants }) {
      ...
      state.filteredApplicantCount =
        state.filteredApplicantCount - applicants.length;
    },
  ...
  actions: {
    async loadJobs(
      ...
      { filterBy, sort, shouldGather = false }
    ) {
      commit("setIsFetching", true);
      try {
        let { jobs, pageCount, filteredJobCount, totalJobCount } =
          await jobService.query(filterBy, sort);
        if (!jobs) return;
    ...
    async loadJob({ commit }, { jobId }) {
      try {
        ...
        loggerService.error(
          "[JobStore] [loadJob] Failed to load job",
          jobId,
          err
        );
      }
    ...
    async loadApplicants(
      ...
      try {
        ...
        loggerService.error(
          "[JobStore] [loadJobs] Failed to load applicants",
          err
        );
      } finally {
        commit("setIsFetching", false);
    ...
    async loadJobToEdit({ commit, rootGetters }, { jobId }) {
      try {
        ...
        const jobToEdit = jobId
          ? await jobService.getById(jobId, user)
          : jobService.getEmptyJob(user);
        commit("setJobToEdit", { jobToEdit });
      } catch (err) {
        loggerService.error(
          "[JobStore] [loadJobToEdit] Failed to load job",
          err
        );
      } finally {
        commit("setIsFetching", false);
    ...
    async addJob({ commit, dispatch }, { job }) {
      try {
    ...
    async removeJob({ dispatch, commit }, { jobId }) {
      try {
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.failRemove("job") },
          { root: true }
        );
      }
    },

    async updateJob({ commit, dispatch, state }, { job, isUndo = false }) {
      try {
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.failUpdate("job") },
          { root: true }
        );
      }
    },

    async toggleArchiveJob({ dispatch, commit, state }, { jobs }) {
      ...
      try {
        ...
        commit(
          "app/setAlertData",
          {
            alertData: msgService.archive(
              "job",
              !!updatedJobs[0].archivedAt,
              updatedJobs.length
            ),
          },
          { root: true }
    ...
    async getExpectedApplicantCount({ commit }, { filterBy }) {
      try {
        const expectedApplicantCount =
          await jobService.getExpectedApplicantCount(filterBy);
    ...
    async removeApplicants({ dispatch, commit, state }, { applicants }) {
      ...
      try {
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.remove("applicant", applicants.length) },
          { root: true }
        );
      } catch (err) {
        commit("setCachedApplicants", { applicants: cachedApplicants });
        loggerService.error(
          "[jobStore] [removeApplicants] Failed to remove applicant:",
          err
        );
        commit(
          "app/setAlertData",
          { alertData: msgService.failRemove("applicant") },
          { root: true }
        );
      }
    },

    async updateApplicants({ commit, dispatch, state }, { applicants }) {
      ...
      try {
        ...
        commit("setCachedApplicants", { applicants: cachedApplicants });
        loggerService.error(
          "[jobStore] [updateApplicants] Failed to update applicant:",
          applicants,
          err
        );
        commit(
          "app/setAlertData",
          { alertData: msgService.failUpdate("applicant") },
          { root: true }
        );
      }
    },

    async toggleArchiveApplicant(
      { dispatch, commit, state },
      { applicants, isAllSelected }
    ) {
      const cachedApplicants = state.applicants;
      ...
      try {
        ...
        commit(
          "app/setAlertData",
          {
            alertData: msgService.archive(
              "applicant",
              !!updatedApplicants[0].archivedAt,
        ...
        commit(
          "app/setAlertData",
          { alertData: msgService.failArchive("applicant") },
          { root: true }
        );
      } finally {
        // commit('setIsFetching', false)
    ...
    async loadJobWithApplicant({ commit, dispatch }, { jobId, applicantId }) {
      commit("setIsFetching", true);
      try {
        ...
        loggerService.error(
          "[JobStore] [loadJobWithApplicant] Failed to add job",
          err
        );
        commit(
          "app/setAlertData",
