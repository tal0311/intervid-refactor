<template>
  <div class="template-picker">
    ...
    <div class="list-wrapper">
      ...
      >
        <button
          class="material-icons scroll-btn start"
          v-if="isStartBtnShowen"
          @click="scrollTo(-1)"
        >
          chevron_left
        </button>
        <div
          class="template-card-preview add"
          @click="$router.push({ name: 'JobEdit' })"
        >
        ...
        >
          <img
            :src="template.coverUrl || defaultImgUrl.jobCover"
            alt="job cover"
          />
          <h2>{{ template.title }}</h2>
        </div>
        <button
          class="material-icons scroll-btn end"
          v-if="isEndBtnShowen"
          @click="scrollTo(1)"
        >
          chevron_right
...
export default {
  ...
  methods: {
    ...
    scrollTo(dir) {
      ...
      const cardCount = Array.from(childNodes).filter(
        (elChild) => elChild.nodeName === "DIV"
      ).length;
      let cardWidth = elScrollableList.children[1].clientWidth;
      const gap = (scrollWidth - cardWidth * cardCount) / (cardCount - 1);
      cardWidth += gap / 2;
      const nextScrollPosition = this.isEnglish
        ? scrollLeft + cardWidth
        : scrollLeft - cardWidth;
      const prevScrollPosition = this.isEnglish
        ? scrollLeft - cardWidth
        : scrollLeft + cardWidth;
      const scrollPosition =
        dir === -1 ? prevScrollPosition : nextScrollPosition;
