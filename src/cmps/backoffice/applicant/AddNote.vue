<template>
  <form class="add-note">
    <textarea v-model.trim="txt" required autocomplete="off" class="note-input" :class="{ active: txt.length }"
      name="txt" :placeholder="getTrans('add-note-on-candidate')" />
...
export default {
  ...
  methods: {
    ...
    checkLimit(ev) {
      ...
      if (value.length <= 2) { //  && value <= totalTimeObj[name]
