...
HTTP.interceptors.request.use(
  (config) => {
    ...
    const isInterview =
      path.includes("interview") && !path.includes("dashboard");
...
async function ajax(endpoint, method = "get", data = null, options = {}) {
  try {
    ...
    if (e.response && !e.config?.url?.includes("record"))
      loggerService.error("Error in http request", e);
    const err = getErr(e);
    if (err.statusCode === 403) {
      store.commit("app/setAlertData", {
        alertData: {
          type: "fail",
          txt: "You dont have enough permissions to complete this action",
        },
      });
      router.go(-1);
    }
