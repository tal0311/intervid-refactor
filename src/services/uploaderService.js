...
function handleNetworkError(fileId) {
  function handleOnReconnect() {
    ...
    loggerService.info(
      `Retrying file upload after network reconnect, file ID: ${fileId}`
    );
...
async function createMultipartUpload(file) {
  ...
  try {
    const { uploadId } = await httpService.get(
      `${ROUTE}/start_multipart?fileKey=${fileKey}&fileType=${file.type}`
    );
    return { key: fileKey, uploadId };
  } catch (err) {
    loggerService.error(
      "[uploaderService] Error when creating multipart upload",
      err
    );
    throw err;
  }
...
function prepareUploadPart(file, { number, key, uploadId }) {
  try {
    return httpService.get(
      `${ROUTE}/sign_multipart?fileKey=${key}&uploadId=${uploadId}&partNum=${number}`
    );
  } catch (err) {
    loggerService.error(
      "[uploaderService] Error when signing upload part url",
      err
    );
    throw err;
  }
...
async function listParts(file, { uploadId, key }) {
  try {
    const { parts } = await httpService.post(
      `${ROUTE}/list_parts?fileKey=${key}&uploadId=${uploadId}`
    );
    return parts;
  } catch (err) {
...
function completeMultipartUpload(file, { uploadId, key, parts }) {
  try {
    return httpService.post(
      `${ROUTE}/complete_multipart?uploadId=${uploadId}&fileKey=${key}`,
      parts
    );
  } catch (err) {
    loggerService.error(
      "[uploaderService] Error when completing multipart upload",
      err
    );
    throw err;
  }
...
function abortMultipartUpload(file, { uploadId, key }) {
  try {
    return httpService.get(
      `${ROUTE}/abort_multipart?uploadId=${uploadId}&fileKey=${key}`
    );
  } catch (err) {
    loggerService.error(
      "[uploaderService] Error when aborting multipart upload",
      err
    );
    throw err;
  }
