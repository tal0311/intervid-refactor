  ...
  <section class="time-event-preview" v-if="timeEvent">
    ...
    <div class="info">
      <div class="top">
        ...
        <p>{{timeEventToShow.txt}}
        </p>
        <time-event-menu v-if="isNote" :idx="idx" @on-remove-note-event="onRemoveNoteEvent" />
      </div>

      <p>{{formattedDate}} · {{timeEventToShow.title}} <span v-if="timeEvent.timeStamp"> at <a
            @click="jumpToTime()">{{timeEvent.timeStamp}}</a></span></p>
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    jumpToTime() {
      ...
      this.$store.commit({ type: 'player/setJumpToPoint', jumpToPoint: +mins * 60 + +secs })
    },
  },
