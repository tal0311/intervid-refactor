<template>
  <section class="overview applicant-overview">
    <h2 class="overview-title">
      <RouterLink v-if="job" class="material-icons" to="/backoffice/job"
        >chevron_left</RouterLink
      >
      {{ overviewTitle }}
    </h2>
    <div class="overview-header">
      ...
      <div class="overview-actions">
        ...
        />
        <share-job
          v-if="job && job.applicantSummary.applicantCount"
          :job="job"
        />
      </div>
    </div>
    ...
    <div
      class="filter-count"
      :class="{ shown: tagList.length || filterBy.txt }"
    >
  ...
  computed: {
    ...
    filterCount() {
      ...
      if (this.filteredApplicantCount > 1)
        return `${getTrans("showing")} ${
          this.filteredApplicantCount
        ...
        return this.lng === "en"
          ...
          : `${getTrans("showing")} ${getTrans("applicant").toLowerCase()} ${
              this.filteredApplicantCount
            }`;
      } else return "";
    },
