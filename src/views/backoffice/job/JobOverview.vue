...
export default {
  ...
  computed: {
    ...
    filterCount() {
      ...
      if (this.filteredJobCount > 1)
        return `${getTrans("showing")} ${this.filteredJobCount} ${getTrans(
          "jobs"
        ).toLowerCase()}`;
      else if (this.filteredJobCount === 1) {
        return this.lng === "en"
          ? `${getTrans("showing")} ${this.filteredJobCount} ${getTrans(
              "job"
            ).toLowerCase()}`
          : `${getTrans("showing")} ${getTrans("job").toLowerCase()} ${
              this.filteredJobCount
            }`;
      } else return "";
    },
  },
  ...
  methods: {
    ...
    async onRemoveSelected(selectedJob) {
      if (selectedJob)
        this.$store.dispatch("job/removeJob", { jobId: selectedJob._id });
      else {
