  ...
  actions: {
    async login({ commit, dispatch }, { userCred }) {
      ...
      try {
        ...
        loggerService.error(
          "[authStore] [login] - Couldn't login user:",
          userCred.email,
          err
        );
    ...
    refreshToken({ commit, state }) {
      ...
      const prm = authService
        ...
        .catch((err) => {
          loggerService.error(
            "[authStore] [refreshToken] - Failed to get refresh token",
            err
          );
          commit("refreshTokenPrm", null);
        });
    ...
    async resetPassword({ commit }, { email }) {
      try {
        ...
        loggerService.error(
          "[authStore] [resetPassword] - Couldn't reset password for",
          email,
          err
        );
        commit("setAuthError", err);
    ...
    async verifyEmail({ commit }, { token }) {
      try {
        ...
        loggerService.error(
          "[authStore] [verifyEmail] - Couldn't verify Email:",
          err
        );
        commit("setAuthError", err);
        throw err;
    ...
    async verifyCode({ commit }, { enteredCode }) {
      try {
        ...
        commit("setAuthError", err);
        loggerService.error(
          "[authStore] [verifyCode] - Verification Unsuccessful",
          err
        );
        throw err;
      }
    ...
    async loginWithOtp({ commit, dispatch }, { email, enteredCode }) {
      commit("loginRequest");
      try {
        ...
        loggerService.error(
          "[authStore] [loginWithOtp] - Couldn't login with Otp - user:",
          email,
          err
        );
        commit("setAuthError", err);
    ...
    async sendVerifyEmail({ commit }) {
      try {
        ...
        loggerService.error(
          "[authStore] [sendVerifyEmail] - Failed to send verification email ",
          err
        );
        commit("setAuthError", err);
        throw err;
