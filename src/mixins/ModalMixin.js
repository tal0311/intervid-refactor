export default {
  ...
  methods: {
    setModalPosition() {
      ...
      this.modalPos = this.mousePos
        ? this.mousePos
        : this.$refs["modal-wrapper"].getBoundingClientRect();

      if (this.mousePos) {
        this.isBottom =
          this.mousePos.y + this.modalHeight > bodyBounding.height;
        ...
        this.isBottom =
          this.modalPos.bottom + this.modalHeight > bodyBounding.height;
        this.modalTop = this.isBottom
          ? this.modalPos.y - this.modalHeight + "px"
          : this.modalPos.y + this.modalPos.height + "px";
        this.modalWidth = this.modalWidth
          ? this.modalWidth
          : this.modalPos.width + "px";
        this.modalInlineStart = this.isEnglish
