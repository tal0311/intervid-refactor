<template>
  <section class="backoffice">
    <div
      class="backoffice-content"
      :class="{ 'no-header': isFullscreenLayout, 'no-overflow-y': isCardView }"
    >
...
export default {
    ...
    isFullscreenLayout() {
      return (
        ...
        (this.$route.fullPath.includes("create") ||
          this.$route.fullPath.includes("details"))
      );
    },

    isMobileToggleShown() {
      return (
        this.$route.name === "ApplicantOverview" ||
        this.$route.name === "JobOverview"
      );
    },
  ...
  methods: {
    async saveApplicant(data) {
      const commitType =
        data.type === "add" ? "addApplicant" : "updateApplicants";
      await this.$store.commit(`job/${commitType}`, {
        applicants: [data.applicant],
      });
      if (data.type === "add")
        this.alertInterviewStatus({ ...data, type: "start" });
    },
