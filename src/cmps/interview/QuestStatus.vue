<template>
  <section class="quest-status">
    <p v-if="!!formattedTimeLeft">
      {{ formattedTimeLeft }} / {{ formattedTimeLimit }}
    </p>
    <div class="progress">
      <div
        class="inner"
  ...
  methods: {
    startInterval() {
      this.timeInterval = setInterval(() => {
        document.title = this.isTimeRunOut
          ? this.getTrans("time-is-running-out")
          : this.defaultTitle;
        this.timeLeft = this.targetTime - Date.now();
        let timeLeftPercent =
          (this.timeLeft / (this.timeLimit * 1000 * 60)) * 100;
