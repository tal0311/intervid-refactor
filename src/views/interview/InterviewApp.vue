...
export default {
  ...
  methods: {
    ...
    updateApplicant(applicant) {
      loggerService.info('[InterviewApp] [updateApplicant] Applicant updated', { applicant })
    ...
    onUploadError(file, err) {
      ...
      if (err.isAxiosError) {
        loggerService.info('[InterviewApp] [onUploadError] Axios error', { err })
        if (!navigator.onLine) {
          this.$store.commit('applicant/setWaitForNetwork', { waitForNetwork: true })
