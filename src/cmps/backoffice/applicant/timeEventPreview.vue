<template>
  <section class="time-event-preview" v-if="timeEvent">
    <i
      :style="{ color: timeEventToShow.color }"
      class="time-event-icon material-icons"
      :class="timeEventToShow.icon"
    >
    ...
    <div class="info">
      <div class="top">
        ...
        <time-event-menu
          v-if="isNote"
          :idx="idx"
          @on-remove-note-event="onRemoveNoteEvent"
        />
      </div>
...
export default {
  ...
  computed: {
    timeEventToShow() {
      return timelineService.getTimeEventToShow(
        this.timeEvent,
        this.applicantName,
        this.jobTitle
      );
