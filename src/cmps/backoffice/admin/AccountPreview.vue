<template>
  <section class="account-preview">
    ...
    <div class="actions">
      <i
        @click.stop="onEditAccount"
        class="material-icons md-dark"
        title="Edit Account"
        >create</i
      >
      <i
        @click.stop="onRemoveAccount"
        class="material-icons md-dark"
  ...
  methods: {
    ...
    onRemoveAccount() {
      const isConfirmed = confirm(
        `Are you sure you want to delete ${this.fullName}\'s account`
      );
      if (isConfirmed)
        this.$store.dispatch("user/removeUser", { userId: this.account._id });
    },
