<template>
  <div class="status-dropdown" ref="modal-wrapper" :class="{ 'full-width': isFullWidth }">
    <button
      class="status-btn"
      :class="{ open: isOpen && !isMobile, recruitment: isRecruitmentStatus, top: isBottom }"
      :style="{
...
export default {
  ...
  computed: {
    ...
    isOpen() {
      return this.modal.type === 'status-picker' && this.modal.data.modalId === this.applicant.id
    },
  },
  ...
  methods: {
    toggleModal() {
      ...
      this.$store.dispatch('app/toggleModal', { type: 'status-picker', data: { modalId } })
    },
