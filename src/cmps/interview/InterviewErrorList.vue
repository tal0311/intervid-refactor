  ...
  <section class="interview-error-list">
    <div v-for="(error, idx) in errors" class="error-preview" :key="idx">
      <div class="error-info">
        <div>
          <i
            ...
            :style="{ color: error.isBlocking ? '#ff5a5a' : '#fffa55' }"
          >
            warning_amber
          </i>
          <p>
      ...
      <div class="error-desc" v-if="openDescIdx === idx">
        <pre>{{
          getTrans(`${error.type}_DESC${getAdditionTag(error.type)}`)
        }}</pre>
      </div>
      <button
        class="material-icons"
        v-if="!error.isBlocking"
        @click="$emit('remove-error', error)"
      >
        close
      </button>
    </div>
    <button
      class="reload-btn"
      v-if="this.errors.length && reloadTxt"
      @click="$emit('reload')"
    >
      {{ getTrans(reloadTxt) }}
    </button>
