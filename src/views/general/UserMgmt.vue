  ...
  <section v-if="userToEdit" class="user-mgmt container">
    <div class="header">
      ...
      <div class="header-txt">
        ...
        <p>{{ userToEdit.companyName || `${getTrans('last-login')} - ` + lastLogin }}</p>
      </div>
      <!-- <lng-menu /> -->
    </div>
    <div class="main-content">
      <form class="user-profile" @submit.prevent="toggleEdit('user', $event)" novalidate>
        <div class="row-header">
          ...
          <button>{{ isUserEdit? `${getTrans('save')}` : `${getTrans('edit')}`}}</button>
        </div>

        <div class="row columns">
          <div class="half">
            <label for="fName">{{ getTrans('first-name') }}</label>
            <p v-if="!isUserEdit">{{ userToEdit.fName || `${getTrans('add-fname')}` }}</p>
            <main-input v-else inputName="firstName" :placeholder="getTrans('first-name')" type="text"
              validate="required" v-model.trim="userToEdit.fName" :onBlur="validateField" :errors="errors"
              styled="basic" />
          </div>
          <div class="half">
            <label for="lName">{{ getTrans('last-name') }}</label>
            <p v-if="!isUserEdit">{{ userToEdit.lName || getTrans('add-lname') }}</p>
            <main-input v-else inputName="lName" :placeholder="getTrans('last-name')" type="text"
              v-model.trim="userToEdit.lName" :onBlur="validateField" :errors="errors" styled="basic" />
          </div>
        </div>
        ...
        <div class="row">
          ...
          <main-input v-else inputName="email" :placeholder="getTrans('add-email')" type="email"
            validate="required|email" v-model.trim="userToEdit.email" :onBlur="validateField" :errors="errors"
            styled="basic" autocomplete="email" />
        </div>

        <div class="row">
          <label for="phone">{{ getTrans('phone-number') }}</label>
          <p v-if="!isUserEdit">{{ userToEdit.phone || `${getTrans('add-phone-number')}` }}</p>
          <main-input v-else inputName="phone" :placeholder="getTrans('add-phone-number')" type="text"
            v-model.trim="userToEdit.phone" :onBlur="validateField" :errors="errors" styled="basic" />
        </div>

        <div class="row">
          <label for="address">{{ getTrans('home-address') }}</label>
          <p v-if="!isUserEdit">{{ userToEdit.address || `${getTrans('add-home-address')}` }}</p>
          <main-input v-else inputName="address" :placeholder="getTrans('add-home-address')" type="text"
            v-model.trim="userToEdit.address" :onBlur="validateField" :errors="errors" styled="basic" />
        </div>
      </form>
      ...
      <form class="company-profile" novalidate @submit.prevent="toggleEdit('company', $event)">
        ...
        <div class="row">
          ...
          <p v-if="!isCompanyEdit">{{ userToEdit.companyName || `${getTrans('add-company-name')}` }}</p>
          <main-input v-else inputName="company-name" :placeholder="getTrans('add-company-name')" type="text"
            validate="required" v-model.trim="userToEdit.companyName" :onBlur="validateField" :errors="errors"
            styled="basic" />
        </div>
    ...
    <div class="profile-links">
      <RouterLink :to="{ name: 'ForgotPassword', params: { isChangePass: true } }">{{ getTrans('reset-password') }}
      </RouterLink>
      <button class="logout-btn" @click="onLogout">{{ getTrans('logout') }}</button>
    </div>
  </section>
</template>
