...
export default function UseModal() {
  ...
  onMounted(() => {
    ...
    modalPos.value = this.mousePos
      ? this.mousePos
      : this.$refs["modal-wrapper"].getBoundingClientRect();
    ...
    if (this.mousePos) {
      ...
      modalInlineStart.value = isEnglish.value
        ? this.mousePos.x - modalWidth.value < 0
          ? "10px"
          : this.mousePos.x - modalWidth.value + "px"
        : bodyBounding.width - this.mousePos.x - modalWidth.value < 0
        ? "10px"
        : bodyBounding.width - this.mousePos.x - modalWidth.value + "px";
    } else {
      isBottom.value =
        modalPos.value.bottom + this.modalHeight > bodyBounding.height;
      modalTop.value = isBottom.value
        ? modalPos.value.y - this.modalHeight + "px"
        : modalPos.value.y + modalPos.value.height + "px";
      modalWidth.value = modalWidth.value
        ? modalWidth.value
        : modalPos.value.width + "px";
      modalInlineStart.value = isEnglish.value
        ? modalPos.value.right - modalWidth.value < 0
          ? "10px"
          : modalPos.value.right - modalWidth.value + "px"
