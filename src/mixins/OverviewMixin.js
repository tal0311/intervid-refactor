...
export default {
  ...
  computed: {
    ...
    tagList() {
      ...
      for (const key in queries) {
        if (key === "daysAgo" && queries[key])
          tags.push({ name: this.getTrans("date"), type: "daysAgo" });
        if (key === "incomplete" && queries[key] !== undefined) {
          const tag =
            queries[key] === "false"
              ? {
                  name: this.getTrans("complete-interviews"),
                  type: "incomplete",
                }
              : {
                  name: this.getTrans("incomplete-interviews"),
                  type: "incomplete",
                };
          tags.push(tag);
        }
        ...
        if (key === "statuses" && queries[key].length)
          tags.push({ name: this.getTrans("status"), type: "statuses" });
      }
      return tags;
    },
  },
  ...
  methods: {
    ...
    onSetFilterByKey(key, value) {
      const filterValue =
        value === this.filterBy[key] && key !== "currPage" ? "" : value;
