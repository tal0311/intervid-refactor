<template>
  <section
    v-if="isArchive"
    class="empty-list archive"
    :class="{ applicant: itemName === 'applicant' }"
  >
    <div class="content-container">
      ...
      <img
        loading="lazy"
        src="https://res.cloudinary.com/intervid/image/upload/v1661182015/Frontend/empty_tnkhmd.svg"
        alt="empty-list"
      />
    </div>
  </section>
  ...
  <section v-else-if="isTemplate" class="empty-list template">
    <div class="txt-container">
      ...
      <a href="/backoffice/template/edit" class="create-btn"
  ...
  <section
    v-else-if="!applicantCount || !totalJobCount"
    class="empty-list create-job"
  >
    <div class="txt-container">
...
export default {
  ...
  computed: {
    ...
    isJobDetails() {
      return (
        this.$route.name === "ApplicantOverview" && !!this.$route.params.jobId
      );
    },
    ...
    archiveTitle() {
      if (this.$route.path.includes("/applicant"))
    ...
    isFilteredCount() {
      if (this.$route.path.includes("/applicant"))
        return !this.filteredApplicantCount && this.applicantCount;
      if (this.$route.path.includes("/job"))
        return !this.filteredJobCount && this.totalJobCount;
      return false;
    },
  },
