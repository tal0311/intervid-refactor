<template>
  <section class="overview job-overview">
    ...
    <table-list :items="jobs" :selectedItemCount="selectedItems.length" :totalItemCount="totalJobCount"
      :shouldGather="shouldGather" :itemsPerPage="filterBy.itemsPerPage" :maxItemCount="totalJobCount"
      :filterBy="filterBy" :sort="sort" :isFetching="isFetching" :isSelected="isSelected"
      :filteredItemCount="filteredJobCount" @select-all="onSelectAll" @sort="onSort" @select="onSelectItem"
...
export default {
  ...
  methods: {
    ...
    async onArchiveSelected() {
      await this.$store.dispatch('job/toggleArchiveJob', { jobs: this.selectedItems })
