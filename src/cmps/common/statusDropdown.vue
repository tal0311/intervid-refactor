  ...
  color: isDisabled ? '#AAADB5' : '#fff',
}" ref="status-btn" @click="toggleModal">
    ...
    <div class="status-modal">
      <button v-for="(status, idx) in statusMap" :key="status.label" :style="{ backgroundColor: status.color }"
        @click="onSetStatus(idx)" :data-label="
          idx === '0'
            ? getTrans('evaluation')
            : idx === '5'
    ...
    <mobile-modal v-if="isOpen && isMobile" cmpName="update status" :applicant="applicant" @on-close="toggleModal"
