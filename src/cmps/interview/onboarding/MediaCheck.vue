    ...
    <div class="check-container">
      <div class="video-wrapper">
        ...
        <div
          ...
          class="recorded-video-btn-container"
        >
          ...
          <div v-if="!!lastRecordedVideo" class="progress">
            <div
              class="inner"
              :style="{ width: lastRecordedVideoProgress + '%' }"
            ></div>
          </div>
        </div>
      ...
      <div class="check-info-container">
        <div
          :class="{ center: isSettingsOpen }"
          class="settings-info-container"
        >
        ...
        >
          ...
          <div class="timer">
            <div class="progress">
              <div
                class="inner"
                :style="{ width: (currTime / previewDuration) * 100 + '%' }"
              ></div>
            </div>
            <p>
              {{ formattedTime.currTime }} / {{ formattedTime.previewDuration }}
            </p>
          </div>
        </div>
          ...
          <div class="device-list">
            <div
              ...
              :class="{
                ...
                'non-blocking':
                  microphoneStatus.isError &&
                  !microphoneStatus.error.isBlocking,
              }"
            ...
            <div
              ...
              :class="{
                ...
                'non-blocking':
                  cameraStatus.isError && !cameraStatus.error.isBlocking,
              }"
            ...
            <div
              ...
              :class="{
                ...
                'non-blocking':
                  screenStatus.isError && !screenStatus.error.isBlocking,
              }"
            ...
            <div
              ...
              :class="{
                ...
                'non-blocking':
                  ...
                  !connectionStatus.error.isBlocking,
              }"
        ...
        <div class="error-info-container" :class="{ center: !!selectedError }">
          ...
          <div v-if="!!selectedError" class="error-info">
            <p
              class="error-title"
              :class="{ 'non-blocking': !selectedError.isBlocking }"
            >
            ...
            <p v-if="selectedError.desc" class="error-desc">
              {{
                getTrans(
                  `${selectedError.type}_DESC${getAdditionTag(
                    selectedError.type
                  )}`
                )
              }}
          ...
          <button
            v-if="selectedError && selectedError.device === 'screen'"
            class="share-screen"
            @click="shareScreen"
          >
          ...
          <device-select
            v-else-if="
              ...
              !!selectedError &&
              selectedError.device === 'microphone'
            "
            :devices="mediaDevice.audio"
            type="microphone"
            :selectedDeviceId="selectedDevice.audio"
...
export default {
  ...
  computed: {
    ...
    microphoneStatus() {
      const errs = this.videoErrors.filter(
        (err) => err.device === "microphone"
      );
      if (errs.length) {
        return {
    ...
    isScreenAns() {
      return this.$store.getters["applicant/job"].quests.some(
        (quest) => quest.ansRule.isScreenAns
      );
    },
  ...
  methods: {
    ...
    async handleFaceCapture(video = this.$refs.video, time) {
      ...
      try {
        ...
        this.isFaceReady = false;
        if (
          !this.videoErrors.find((error) =>
            videoErrorTypes.includes(error.type)
          )
        ) {
          loggerService.error(
            "[MediaCheck] [handleFaceCapture] Could not capture applicant face",
            { err }
          );
    ...
    updateProgressTime(ev) {
      const lastRecordedVideoProgress = Math.round(
        ((ev.srcElement.currentTime * 100) / this.previewDuration) * 100
      );
      this.lastRecordedVideoProgress =
        lastRecordedVideoProgress > 93 ? 100 : lastRecordedVideoProgress;
    },
    ...
    getAdditionTag(errorType) {
      ...
      const browserAdditionErrors = [
        videoErrorMap.DENIED_CAM_ACCESS.type,
        videoErrorMap.DENIED_MIC_ACCESS.type,
      ];
      if (browserAdditionErrors.includes(errorType)) {
        ...
        if (browsers.includes(browserName))
          return "_" + browserName.toUpperCase().replace("-", "_");
      }

      const { os } = this.browser;
      const deviceAdditionErrors = [
        videoErrorMap.MIC_NOT_FOUND.type,
        videoErrorMap.CAMERA_NOT_FOUND.type,
      ];
      ...
      if (
        errorType === screenErrorMap.DENIED_SCREEN_ACCESS.type &&
        os === "Mac OS"
      ) {
