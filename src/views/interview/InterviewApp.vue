...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query.demo)
      this.$store.commit({ type: "applicant/setIsPreview", isPreview: true });
  },
    ...
    isBrowserVersionSupported() {
      ...
      if (os === "iOS") {
        ...
        const iosVersion = Number(
          agent.substring(start + 3, 3).replace("_", ".")
        );
  ...
  methods: {
    ...
    async initInterview() {
      ...
      await this.loadJob();
      if (!this.job)
        return this.setInterviewErr({ type: "INTERVIEW_NOT_FOUND" });
      this.loadApplicant();
      this.setMeta();
    },
    setMeta() {
    ...
    onUploadSuccess(file) {
      loggerService.info(
        "[InterviewApp] [onUploadSuccess] file uploaded successfully",
        { file }
      );
      const fileKey = `${this.job._id}/${file.meta.name}`;
      this.saveAsset(fileKey, file.data.interviewData);
      this.failedFiles = this.failedFiles.filter(
        (fileId) => file.id !== fileId
      );
      if (
        this.totlaAssetsCount - this.totallyFailedfiles.length ===
        this.updatePrms.length
      ) {
        loggerService.info(
          `[InterviewApp] [onUploadSuccess] Last asset finished uploading`
        );
        this.doneInterview();
      }
    },
    ...
    onUploadError(file, err) {
      ...
      if (err.isAxiosError) {
        ...
        if (!navigator.onLine) {
          ...
          loggerService.info(
            "[InterviewApp] [onUploadError] Axios error - network error",
            { err }
          );
      ...
      if (retryCount >= this.MAX_TRY_COUNT) {
        ...
        if (
          ...
          loggerService.info(
            `[InterviewApp] [onUploadError] Last asset finished uploading`
          );
          this.doneInterview();
        }
        return;
    ...
    async doneInterview() {
      try {
        ...
        loggerService.info(
          "[InterviewApp] [doneInterview] All upload updates fulfilled"
        );
      } catch (err) {
        loggerService.error(
          "[InterviewApp] [doneInterview] error when waiting for prms",
          err,
          {
    ...
    async saveApplicant() {
      ...
      loggerService.info(
        "[InterviewApp] [saveApplicant] Save full applicant (add submitted time event)",
        { applicant }
      );
      this.updateApplicant(applicant);
      socketService.emit(
        SOCKET_EMIT_DONE_INTERVIEW,
        getEmitData("submit", this.job, applicant)
      );
    },
    ...
    handleQuit() {
      ...
      loggerService.info(
        "[InterviewApp] [handleQuit] Save full applicant (add quited time event)",
        { applicant }
      );
      socketService.emit(
        SOCKET_EMIT_DONE_INTERVIEW,
        getEmitData("quit", this.job, applicant)
      );
      ...
      navigator.sendBeacon(
        url,
        JSON.stringify({ applicant, jobId: this.job._id })
      );
      loggerService.info(
        `[InterviewApp] [handleQuit] Applicant ${this.applicant.id} has quit the interview`
      );
    },
  },
