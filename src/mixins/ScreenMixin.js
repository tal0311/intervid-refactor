  ...
  methods: {
    async initScreenMixin() {
      ...
      try {
        ...
        if (browserName === "safari") {
          ...
          loggerService.info(
            "[ScreenMixin] [initScreenMixin] Safari screen share alert added"
          );
        }
      } catch (err) {
        loggerService.error(
          "[ScreenMixin] [initScreenMixin] Denied screen access",
          { err }
        );
      ...
      this.addStreamStopListener().then((ev) => {
        loggerService.error(
          "[ScreenMixin] [initScreenMixin] Candidate denied screen access - event: ",
          { ev }
        );
        this.addScreenError(screenErrorMap.DENIED_SCREEN_ACCESS);
      });
    ...
    stopScreenRecording(interviewId, questId) {
      return new Promise((res) => {
        ...
        loggerService.info(
          "[ScreenMixin] [stopScreenRecording]",
          { interviewId },
          { questId }
        );
      });
    ...
    removeScreenError(errorToRemove) {
      if (this.screenErrors.some((err) => err.type === errorToRemove.type)) {
        this.screenErrors = this.screenErrors.filter(
          (error) => error.type !== errorToRemove.type
        );
        loggerService.info(
          "[ScreenMixin] [removeScreenError] Screen error removed",
          { errorToRemove }
        );
      }
    },
