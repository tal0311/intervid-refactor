<template>
  <form class="add-note">
    ...
    <div class="btn-container">
      ...
      <div
        v-if="verifyPerm(advancedPermsMap.BOOKMARKS)"
      ...
      >
        ...
        <button
          type="button"
          class="ts-btn material-icons"
          :class="getActiveClass"
          @click="openTimeStamp"
        >
          bookmark
        </button>
        <div v-if="isTsOpen && isWarning" class="video-limit-warning">
          Outside limits
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async addNote() {
      ...
      if (this.isTsOpen && this.tsMins && this.tsSecs)
        note.timeStamp = `${this.tsMins}:${this.tsSecs}`;
      this.txt = "";
      this.tsMins = null;
      this.tsSecs = null;
      this.isTsOpen = false;
    ...
    checkLimit(ev) {
      ...
      if (value.length <= 2) {
        ...
        if (
          name === "tsSecs" &&
          value <= 59 &&
          +value + this.tsMins * 60 <= totalVideoSecs
        ) {
          this[name] = `${value}`.padStart(2, 0);
          return;
        } else if (
          name === "tsMins" &&
          +value <= totalTimeObj.tsMins &&
          +value * 60 + +this.tsSecs <= totalVideoSecs
        ) {
          this[name] = `${value}`.padStart(2, 0);
          return;
        }
